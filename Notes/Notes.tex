...
\begin{lemma} \label{Homo Harmonic}
    The unique harmonic function on the disc with h BCs is $f=0$. 
\end{lemma}
\begin{proof}
...
\subsection{Compatibility Conditions/Regularity}
We have been using $\GN:L^2(D)\to\mathcal{N}(D)$ to find $f$ from $s = \lap f$ with the requirement that $\iint_D s\ dA = 0$ from Lemma \ref{Neumann Problem}. So really, we mean $\GN:\mathcal{F}\to\mathcal{N}$, where $s\in\mathcal{F}\subset L^2(D)$ if and only if $\iint_D s\ dA = \int_{\p D} \np f\cdot\bm{n}\ dl = 0$.

For $\lap\phi = \np\cdot\bm{\nu}_\perp$, this works because we have the boundary condition $\bm{v}\cdot\bm{n} = 0$ on $\p Q$, or $\bm{v}_\perp\cdot\bm{n}_\perp = 0$ on $\p D$: $\iint_D \np\cdot\bm{\nu}_\perp\ dA = \int_{\p D} \bm{\nu}_\perp\cdot\bm{n}_\perp\ dl = 0$, so $s\in\mathcal{F}$.

When inverting $\lap\p_\tau\phi = \np\cdot\p_\tau\bm{\nu}_\perp$, we instead require $\iint_Q \np\cdot\p_\tau\bm{\nu}_\perp\,dA = \int_{\p Q} \p_\tau\bm{\nu}_\perp\cdot\bm{n}_\perp\,dl = 0$. If $\bm{\nu}_\perp(t)\in C^\infty$ varies smoothly with time, we may take $\p_\tau\pth{\bm{\nu_\perp}\cdot\bm{n}_\perp} = \p_\tau\bm{\nu}_\perp\cdot\bm{n}_\perp = 0$. If $\bm{\nu}_\perp$ does not have some notion of ``regularity'' in time however, this is not necessarily the case, and $\np\cdot\p_\tau\bm{\nu}_\perp$ may not $\in\mathcal{F}$. This distinction becomes clear when we consider that our expression for $\p_\tau\bm{\nu}_\perp$ in \eqref{nuperpevolution}; setting $\p_\tau\bm{\nu}_\perp\cdot\bm{n}_\perp = 0$ seems to imply additional conditions on $\beta_\para$, $\bm{\beta}_\perp$, $\bm{\nu}_\perp$, and $\pi$. Thus $\p_\tau s\in\mathcal{F}$, only follows from $s\in\mathcal{F}$ if $s$ obeys the above ``compatibility condition.''

\subsection{Boundary Conditions for $\beta_\para$}
Divergence theorem argument: Consider a flux tube $T\subset Q$ going from $Z_1$ to $Z_2$. Because $\nabla\cdot\bm{B}=0$,  
\begin{align*}
\iiint_T \pth{\nabla\cdot \bm{B}} dV = \iint_{\p T} \pth{\bm{B}\cdot \bm{n}} dA = 0.
\end{align*}
...
\begin{align*}
\iint_D B_z\pth{X,Y,Z_2}\,dA = \iint_D B_z\pth{X,Y,Z_1}\,dA
\end{align*}
for arbitrary $Z_1$ and $Z_2$, which implies $\iint_D \p_ZB_z\,dA = 0$. When we set $B_z = B_0\pth{1+ \ep \beta_\para}$, this condition becomes $\iint_D \p_Z\beta_\para\,dA=0$. This is necessary when we try to invert $\lap\Phi = -\ep\p_Z\beta_\para$, which requires $\p_Z\beta_\para\in\mathcal{F}$.

$\beta_\para$ has a poloidal average, $\bar{\beta}_\para = \frac{1}{A}\iint_D \beta_\para\,dA$.
This lets us decompose $\beta_\para\pth{X,Y,Z} = \bar{\beta}_\para\pth{Z} + \til{\beta}_\para\pth{X,Y,Z}$, where $\til{\beta}_\para\in\mathcal{F}$ obeys $\iint_D \til{\beta}_\para\,dA = 0$ (this is because $= \iint_D \beta_\para\,dA - \iint_D \bar{\beta}_\para\pth{Z}\,dA = \bar{\beta}_\para A - \bar{\beta}_\para A = 0$). Then, the condition above becomes $\p_Z\bar{\beta}_\para = \frac{1}{A}\iint_D \p_Z\beta_\para\,dA = 0$, implying that $\bar{\beta}_\para\pth{Z}=$ constant, so $\iint_D \p_Z\beta_\para\,dA = \iint_D \p_Z\til{\beta}_\para\,dA=0$.
